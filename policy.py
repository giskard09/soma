...
- directed_outreach: mass messages, DM automation, unsolicited contact
- fund_ops_foreign: on-chain ops with wallets not owned by requester
- disinformation: factual-looking content without sources on sensitive topics
...
def _log(request_text: str, result: dict):
    entry = {
        "ts": int(time.time()),
...
def check_policy(request_text: str) -> dict:
    """
    Returns: {decision, category, reason, policy_version, provider}
    decision ∈ {accept, reject, escalate}
    """
    if not request_text or not request_text.strip():
        return {"decision": "reject", "category": None,
